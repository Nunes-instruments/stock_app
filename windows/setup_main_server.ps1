...
New-Item -ItemType Directory -Force -Path $DataDir,(Join-Path $DataDir 'db'),(Join-Path $DataDir 'uploads'),(Join-Path $DataDir 'processed'),(Join-Path $DataDir 'exports'),(Join-Path $DataDir 'backups') | Out-Null
...
foreach($name in @('uploads','processed')) {
    $from = Join-Path $SourceDir $name; $to = Join-Path $DataDir $name
    if(Test-Path $from){ Get-ChildItem $from -File | ForEach-Object { $target=Join-Path $to $_.Name; if(-not(Test-Path $target)){Copy-Item $_.FullName $target} } }
}
$cat = Join-Path $SourceDir 'storage_categories.json'
if((Test-Path $cat) -and -not(Test-Path (Join-Path $DataDir 'storage_categories.json'))){Copy-Item $cat (Join-Path $DataDir 'storage_categories.json')}

if (-not (Get-Command git.exe -ErrorAction SilentlyContinue)) { throw 'Git for Windows is required. Install Git, then run SETUP_MAIN_SERVER.bat again.' }
if (-not (Get-Command py.exe -ErrorAction SilentlyContinue) -and -not (Get-Command python.exe -ErrorAction SilentlyContinue)) { throw 'Python 3 is required. Install Python 3.12, then run setup again.' }
...
if (Test-Path (Join-Path $AppDir '.git')) {
    ...
    if($LASTEXITCODE -ne 0 -or -not (Test-Path (Join-Path $AppDir 'flask_app.py'))){ throw 'GitHub repository has not been published yet. Run PUBLISH_TO_GITHUB.bat from the updated package first, then run SETUP_MAIN_SERVER.bat again.' }
}
...
# Firewall rule is idempotent.
$rule = Get-NetFirewallRule -DisplayName 'NUNES Stock Server 5000' -ErrorAction SilentlyContinue
if(-not $rule){ New-NetFirewallRule -DisplayName 'NUNES Stock Server 5000' -Direction Inbound -Protocol TCP -LocalPort 5000 -Action Allow | Out-Null }

# Desktop shortcuts.
$desktop = [Environment]::GetFolderPath('Desktop')
$ws = New-Object -ComObject WScript.Shell
$open = $ws.CreateShortcut((Join-Path $desktop 'NUNES Stock - Owner.lnk'))
$open.TargetPath = "$env:SystemRoot\System32\cmd.exe"
$open.Arguments = '/c start "" "http://127.0.0.1:5000"'
$open.WorkingDirectory = $AppDir
$open.Description = 'Open NUNES Stock owner view on the main server'
$open.Save()
...
$ip = $null
if($route){ $ip = Get-NetIPAddress -InterfaceIndex $route.InterfaceIndex -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object {$_.IPAddress -notlike '169.254.*'} | Select-Object -First 1 -ExpandProperty IPAddress }
Write-Host ''
